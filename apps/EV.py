...
def app():
    ...
    with st.expander('Описание:'):
        st.write("""
        Задача оптимизации — рассчитать суточный график зарядки электромобилей (**EV**), с учётом ограничения по установленной мощности подстанции
        (**S_upper_limit, кВА**), который:
        ...
        - **S_upper_limit** — ограничение по установленной мощности подстанции, кВА
        - **P_max** — максимальная мощность зарядки EV, кВт — одно значение для всех EV
        - **n_EVs** — количество EV
        - **data_houses** — название файла в формате csv с данными по потреблению домов за сутки
        - **data_EVs** — название файла в формате csv с данными об электромобилях
        #### 2. Опциональные параметры:
        - **house_koeff** — коэфф. для пропорционального увеличения/уменьшения общего потребления домов
        - **EV_koeff** — коэфф. для пропорционального увеличения/уменьшения общего требуемого потребления EV
        - **use_lighting** — указание на то, учитывать ли затраты эл.энергии на освещение — 7.2 кВт с `17:00` до `9:00` след. дня.
           - **1** — учитывать;
           - **0** — не учитывать.
        - **key** — используется, если не решается текущая задача оптимизации — с текущим значением предела по установленной мощности.
          - **1** — решение новой задачи оптимизации — с другим, бОльшим пределом установленной мощности — **S_upper_upper_limit**.
          - **0** — решение старой задачи оптимизации, с **S_upper_limit**.
        - **S_upper_upper_limit** — новый верхний предел по установленной мощности для задачи оптимизации
        - **use_quick_charges** — указание на то, учитывать ли три быстрые зарядки по 50 кВт:
          ...
          - зарядка 3 - с `12:00` до `13:00`; с `13:00` до `15:00`; с `16:00` до `18:00`; с `21:00` до `23:00`; с `11:00` до `12:00` 
            - use_quick_charges = **1** — учитывать быстрые зарядки.
            - use_quick_charges = **0** — не учитывать.
            
        Результат работы программы — три графика:
        - общее потребление домов (кВА) в течение суток;
        - общее потребление домов + EV (кВА) в течение суток без оптимизации;
        - общее потребление домов + EV (кВА) в течение суток с оптимизацией.
        
        Задача оптимизации будет иметь решение (в виде профилей потребления электромобилей) далеко не при всех значениях параметров.
        В частности, задача не решается, если максимальная мощность зарядки EV и возможное время зарядки EV таковы, что все EV не успевают зарядиться до требуемого уровня за доступное время.
        Возможное решение — увеличить мощность зарядки EV и доступное время зарядки, не разряжать батарею ниже определённого уровня.
        
        Также важно понимать, что при задании количества EV (**n_EVs**) они выбираются из всех имеющихся EV случайным образом при каждом запуске программы.
        Каждый такой набор EV будет характеризоваться своими запросами по зарядке и доступным временем для зарядки.
        Поэтому задача оптимизации может иметь решение для одного запуска программы — с одним набором EV — и не иметь решения при следующем запуске программы — с другим набором EV.
        """)
