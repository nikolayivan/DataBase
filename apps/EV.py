...
def app():
    ...
    with st.expander('Описание:'):
        ...
        ***
        ...
        - превышение предела установленной мощности, в %;
        - указание на то, учитывать ли три быстрые зарядки по 50 кВт (галочка - "Быстрые зарядки"):
        ...
        - указатель, который определяет выбор типа файла с данными по потреблению домов, который пользователь загрузит в программу.
           - Можно загрузить файл с данными по суточному потреблению одного дома. На основе этих данных программа рассчитает примерный профиль потребления желаемого количества домов. Эта опция доступна, если нажать галочку "Использовать данные по одному дому".
        ...
        ***
        ...
        - **Данные по потреблению домов/квартир/домохозяйств:**
         ...
         Можно загрузить файл с данными по суточному потреблению одного дома. На основе этих данных программа рассчитает примерный профиль потребления желаемого количества домов. Эта опция доступна, если нажать галочку "Данные по одному дому".
        ...
        - **Данные об электромобилях:**
           ...
           - Данные для каждого электромобиля:
              ...
              - планируемое время выезда (с точностью до часа — например, `9:00` или `22:00`. Считаем, что EV выезжают не позже `10:00` второго дня);
        ...
        *Шаблоны файлов доступны для скачивания ниже.
        ...
        ***
        ...
        with open("./data/data_houses_example.csv", "rb") as file:
            col1.write("Скачать шаблон для нескольких домов:")
            col2.download_button(label="⬇️ Скачать", data=file, file_name='data_houses_example.csv', mime="csv",)

        with open("./data/1 house 24 hours.csv", "rb") as file:
            col1.write("Скачать шаблон для одного дома:")
            col2.download_button(label="⬇️ Скачать", data=file, file_name='1 house 24 hours.csv', mime="csv",)

        with open("./data/data_EV_example.csv", "rb") as file:
            col1.write("Скачать шаблон для электромобилей:")
            col2.download_button(label="⬇️ Скачать", data=file, file_name='data_EV_example.csv', mime="csv",)
...
#         with col2:
        ...
        key_houses = st.checkbox('Использовать данные по одному дому')
        
        col1,col2 = st.columns(2)
        with col1:
            data_houses = st.file_uploader('Загрузите данные по потреблению домов за сутки:', type='csv')
        with col2:
        ...
        submitted = st.form_submit_button("🚀Раcсчитать")
